...
| 2.10.f follow-up: *Read all* honours the cap | `73cf827` (from Coder B's `d715b4b`) | the extract route's `start` takes a slot or defers (202, `waiting`); `retry-dispatch`/`restart-expired` uncapped with reasons; advisory lock before row lock (a deadlock otherwise); a deferred `failed` run reset to `pending`; `ResolvedRun.waitingForSlot`; the review's *Read all* reports "3 reading · 8 waiting for a slot" and excludes promised reads; +7 db, 4 component tests; full suite 1,644 green with the database tier on the coder's branch | driven through the real route against the sandbox with the publisher stubbed (3 queued / 2 waiting, chips agreeing) — a browser press would publish real charged reads, so none was made | `3594362` deployed on `spec-builder-app-rho.vercel.app`, read back from `/api/auth/me` |
| **2.10.a** the bills variance rows, nine of nine | `2fd35a1`…`7584afe` (eleven commits, from Coder C's `stage2-b`) | synthetic `.xlsx` bytes from a committed builder; 4 new test files + 5 extended (+68 tests). Code changed on four rows: unknown headings refused naming the bill's own words on the review screen; a two-row header read as one; ONE code fold everywhere (the reconcile's divergence would have retired a record over `S.201` vs `S-201`); legacy spreadsheets refused in the browser with *save as .xlsx*. Flagged, not changed: no-qty column says *quantity not given* on row and tab; a PDF bill refused at classify; two bills in a pack named. Pinned only: subtotal rows skipped; the 300-line review at 351 ms alone, 0.9–2.2 s under a full run | held by the pure, component and db tiers; the browser drive of a refused bill and a no-qty bill is the walk script's next step, not yet written | `98125fc` deployed on `spec-builder-app-rho.vercel.app`, read back from `/api/auth/me` |
| **2.10.b/c** the drawings and email variance rows, six of six | `464b100`…`7572828` + merge `3cd7ea6` (Coder D's `stage2-a`) | +27 pure, 5 component, 6 db/route tests; fixtures from `build-pdf.mjs`. Code changed on five rows: six arrays stop failing a paid read over a hint; a feet-and-inches compound refused whole (it half-read as 1); `.msg` refused at registration (it was being read as binary); attachments listed and downloadable, registering nothing; a 600-page cap before the call (first set at 100 and corrected against the API reference — a test pins it). Proved only: rotated-page crop maths. Row 6's premise ("it would hang") was false — a 400 came back after an attempt was spent | held by the pure, component and db tiers; no model call anywhere | `3cd7ea6` fast-forwarded onto staging; `98125fc` deployed, read back from `/api/auth/me` |
| **2.11** a confidently routed email is assigned automatically | `5b2b9f3` (Coder E's `f796d29`, cherry-picked WITH the gate amendment in the same commit — Max: "yes, amend", 2026-09-21) | `autoAssignDecision` (pure, two signals only, actor `system:router`); the Graph path narrowed from every `assigned` outcome to those two; placement failures hold the message with the reason; the inbox's consent sentence, the `assigned automatically · <signal>` chip, *Waiting for a slot*, weak-signal rows naming the project without choosing it; 12 pure + 4 component + 10 db tests (the real `ingestGraphMessage` with queue, blob and Graph stubbed); full suite 1,735 green on the coder's branch | dev server: the inbox reads "Mail addressed to a project's inbox, or forwarded from it, is assigned and read automatically the moment it arrives — one charged model call each…", held rows say why, 1920×1080 screenshot clean. `qa:fake-inbox --apply` re-recorded 13 fixtures: every sender- and subject-routed one HELD; none is inbox-addressed, so none auto-assigned — adding one is a fixture that starts a charged read, Max's call. Graph is `disabled` everywhere, so nothing auto-assigns in production today | `526164f` deployed on `spec-builder-app-rho.vercel.app`, read back from `/api/auth/me`; the deployed inbox renders the consent sentence |
| **2.10.d/e** the records, phases and finishes rows, eight of eight | `3f56808`…`4dc14fa` (eleven commits, Coder G's `stage2-b`) | 20 db/route + 15 component + 4 pure tests. Code changed on four rows: an uncategorised record's phase-table cell says *no checklist and nothing to count — not nothing outstanding* (it opened the LEVEL sentence); a zero-ref record reads *no client ref* (it printed the same em dash as an empty column); *quantity not allocated* in one wording on all three list screens and the costing note asserted to count; **an export for a retired phase is refused with a 409 in words** (it composed an empty workbook named after the phase). Pinned: two finish codes never merge (fold AND index), a code with two descriptions stays a conflict the finishes page names, a code with no description is a bare TBC row. The plan's "a person cannot set a configuration's quantity" was FALSE since 0028; corrected in CLAUDE.md | held by the db, route, component and pure tiers; no fixture left on the sandbox | `6206e7d` deployed on `spec-builder-app-rho.vercel.app`, read back from `/api/auth/me` |
| F.1 two database-tier runs at once | `7d34ac9` + `85e8a8a` (Coder F) + `52b5e39` (Fable, three literals in Coder G's new files) | `qaNumber` in `db-tier.ts`, 30 files routed through it, every same-file lookup by number too; mailbox, subscription id, scratch table and `shared_inbox` fixtures carry the suffix; dead pre-cleans removed. PROVED: two full db-tier passes at once, both `133 passed · 1776 tests`, exit 0 — the same pair failed 3 and 6 tests before | — | `36a5fe9` deployed, read back from `/api/auth/me` |
| F.2 a deferred read on the three single-document screens | `f91f43c` | `waitingForSlot` on the GET payload; chip, sentence and a secondary *Read it now*; a 202 press reported as an info notice after the reload; 5 component tests + 1 db | held by tests; not driven in a browser (a deferred run needs a pack at the cap, which a real registration would charge) | `36a5fe9` deployed, read back |
| F.3 a scanned PDF refused at classify | `91360b6` | `pdfHasTextLayer` (certain-or-null), `scannedPdfRefusal`, the classify route checks before the model; 10 pure tests, three synthetic page shapes in `build-pdf.mjs` | held by tests; no model call | `36a5fe9` deployed, read back |
