...
Max, 2026-09-20: *"as per make-it-work-2026-09-19.md lets build stage 2."*
Stage 1 is on `staging` at `da863b7` and promoted to pilot; Matthew's
screenshots have not arrived. The plan's own rule for this stage is that it
is *"built one item at a time, in the order Matthew's first session makes
right"* — with no session yet, the order below is dependency order from the
§6 table, and it changes the day his findings arrive (§7.7). Nothing here
re-decides an item; the four design-first items already have their designs in
§5.3, §6.5, §6.6 and §6.8, and "build stage 2" is taken as covering them the
way "go with 1a" covered §5.2. A coder who finds a design wrong stops and
says so.

| | |
|---|---|
| **Not started, and why** | **2.1 / 2.2** — the BWS account (0.1) still does not exist; nothing else here can start them. **2.9** — a proposal to Matthew, not code; it goes in the questions message below. **2.12** — another session's plan, sequenced after 2.3. **2.13** — an email to Tony, drafted for Max. **2.11** is built LAST, behind the rate cap, and does not land until Max has written the gate amendment into `CLAUDE.md` himself (§2.6) |
| **Order** | **Round 1:** 2.4 (coder A, small, first and alone — the area select is what 2.3 and 2.5 both filter by) then 2.5 (coder A: the EMAIL regrouping and the internal recipient), beside 2.3 (coder B: the infill screen, starting with the loader's cost at 300 lines) then 2.7 (coder B: the by-question tab on the same screen). **Round 2:** 2.6 (coder A: the dimension note, migration 0034) beside 2.8 step 1 → 2.10.f → 2.10.g (coder B: the project-wide fold, the per-batch in-flight cap, the failure-surface sweep). **Round 3:** the variance matrix, one small brief per row, bills first (header synonyms refused with the words accepted; no quantity column; `.xls`/`.csv`; subtotal rows; the 300-line timing), then inches on a drawing, then `.msg`; and 2.11 prepared with its amendment text for Max |
| **The 300-line test is a prerequisite, not a check** | The 300-line overview renders in 10.4s (`found-in-use.md`, 2026-09-20) and `loadOutstanding` over 503 records is the named weight. 2.3 and 2.7 are list screens over that loader, and §7.4a's two seconds is a row of their DoD. So coder B's first act is to MEASURE `loadOutstanding` on DEMO-300 (`a88aed3e…`, 300 lines, 35 areas) and to make it fast enough — or to say in words how many rows there are and how many are shown. The number before and after goes in the commit body |
| **Coders** | Two at once, never three. Worktrees are OUTSIDE iCloud — `~/dev/worktrees/s2-a` and `s2-b`, branches `stage2-a` / `stage2-b` off `staging`, `node_modules` symlinked to the main checkout rather than installed (the 2026-09-20 load-average lesson), `.env.local` copied in so the db tier RUNS in the worktree. Neither runs `db:migrate`, pushes, or touches `CLAUDE.md` |
| **Files — A** | 2.4: new `src/lib/area-filter.ts` (the case/whitespace fold and the grouping, pure) + test, a new `src/components/ui/AreaSelect.tsx`, wired into the chase screen's filter bar (`src/app/dashboard/drafts/page.tsx` — the FILTER BAR only, not the table) and the phase table (`SpecTable.tsx`, `src/app/api/records/route.ts` if the areas must come from the server). 2.5: `src/lib/chase-template.ts` and its test, `src/lib/chase-drafts.ts` ONLY where the coverage rows need the area and the recipient kind, the draft generate/edit routes under `src/app/api/drafts/`, the recipient picker on the chase screen. 2.6: `db/migrations/0034_dimension_note.sql`, `src/lib/dimensions.ts`, `src/lib/record-atoms.ts`, `src/lib/bws-export.ts` (the dimensions cell's input only), `src/lib/export-check-sheet.ts`, `src/app/api/records/[id]/route.ts`, `RecordDetails.tsx`, `src/lib/snapshot-diff.ts` if the atom needs a key; tests in the pure, db and route tiers |
| **Files — B** | 2.3 / 2.7: new `src/app/dashboard/projects/[id]/infill/page.tsx`, new `src/components/infill/*`, `src/components/drafts/ChaseQuestionTable.tsx` ONLY to extract the shared table body (the tick column is the difference), `src/lib/chase-drafts.ts` for the loader's cost, `src/lib/chase-grouping.ts` for the by-question grouping (a second pure grouping beside `groupIntoLines`, same input), `src/lib/next-step.ts` (the infill screen is a next step), a new `tools/measure-outstanding.ts` (§7.3), tests in the component, db and route tiers. 2.8 step 1: `RecordChecklist.tsx` and its test — NOTHING in the data. 2.10.f: `src/lib/extraction-dispatch.ts`, `src/app/api/imports/route.ts`, the queue consumer, `tests/lib/extraction-timing.test.ts`. 2.10.g: every `catch` under `src/app` and `src/components`, one component test per screen |
| **Where A and B meet** | The chase screen. A owns the filter bar and the recipient picker; B owns the table. 2.4 lands on `staging` first and B rebases before touching the drafts page. `loadOutstanding` is B's until 2.3 lands, then A's for 2.5 — never both at once |
| **Nobody's** | `CLAUDE.md`/`AGENTS.md`, `loadExportScope`, `composeRowCells`' order of precedence, `promote-answers.ts`, every migration before 0034, the extraction prompts and tool schemas (`anthropic.ts`, `extraction-schema.ts` — a re-read is charged) |
| **Migrations** | ONE: `0034`, `spec_records.dimension_note text` with a length and a no-newline CHECK. Written by coder A, applied to the sandbox by Fable after `db:backup`. 2.8 step 2's `scope` column is NOT in this stage (§6.8: only if Matthew asks again). 2.3 needs no table — a meeting is a change set |
| **`db:migrate` / sandbox** | Fable. No coder runs it |
| **Fable's own** | this brief; the §2.3 review of every diff before its checks; the four checks with the db tier REQUIRED before every cherry-pick, each gated on its own exit code; the §7.4a screenshots at 1920×1080 and 1440×900 and the 300-line walk of every list screen touched, never by the coder; `CLAUDE.md`/`AGENTS.md` per §2.6 — the chase section says the EMAIL is grouped by question × area and the SCREEN by item, the infill screen gets its load-bearing section, the palettes section is left saying "never had it" because that is still true; a dated line per item in `docs/plans/README.md`; `found-in-use.md`: finding 10 marked for step 1, finding 3 left open until Matthew answers, the auto-assign entry updated with the cap's landing; the first-session script gains a step for the infill screen and one reading the regrouped email; the questions message — §11 A, C, D, F plus 2.13's ask of Tony — drafted for Max to send |
...
| Deployed | `spec-builder-app-rho.vercel.app` read back from `/api/auth/me` at `3f8511d`, `f288ea9`, `9715b0f`, `237e1ba`, `3594362`, `98125fc`, `526164f`, `6206e7d`, and `36a5fe9` (the whole of Stage 2's code) — every one read back from `/api/auth/me` |
