...
| **iCloud writes `" 2"` copies into `.next` and typecheck reads them** (FIU 2026-09-20) | **Max** — environment, not code | The checkout lives under `Mobile Documents/com~apple~CloudDocs`. Moving it, or excluding `.next` from sync, is his call. `find .next -name "* 2.*" -delete` is the workaround, and the failure reads as a regression in whatever commit is under test |
