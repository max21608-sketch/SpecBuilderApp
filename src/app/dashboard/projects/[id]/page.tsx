...
export default function ProjectOverviewPage() {
  ...
  const load = useCallback(async () => {
    const res = await apiFetch<{ project: Project; documents: DocumentRun[] }>(
  ...
  // The designer codes this project's records carry that nobody is yet. The
  // chase screen computes the same thing from its inventory; here it comes off
  // the records list, so the canonical contacts home can show the gap too.
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <form onSubmit={save} className="mt-4 border border-neutral-200 rounded-lg bg-white p-4">
        ...
        <div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-3">
          <label className="text-sm text-neutral-600">
            ...
            <span className="block mt-1 text-xs text-neutral-500">
              Fixed. Every record number and stored chase is labelled with it.
            </span>
          </label>
          <label className="text-sm text-neutral-600">
            Name
        ...
        <p className="mt-1 text-xs text-neutral-500">
          Copied on every chase email drafted for this project. Leave it blank for no Cc — that is a real choice,
          and the draft screen says which one is in force.
        </p>
        <input
          {...field("sharedInbox")}
          type="email"
      ...
      <p className="mt-1 text-xs text-neutral-500">
        Who a chase email can be addressed to. The designer code matches the BOQ&rsquo;s own wording and is what
        routes a record&rsquo;s questions to a person.
      </p>
      {contacts === null ? (
            ...
            <li key={run.id} className="px-4 py-3 flex items-center gap-4 text-sm">
              <div className="min-w-0 flex-1">
                <p className="text-neutral-900">
                  ...
                  <span className="text-neutral-500"> · {SOURCE_LABELS[run.source_kind] ?? run.source_kind}</span>
                </p>
